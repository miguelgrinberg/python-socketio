...
class TestAsyncManager:
    ...
    async def test_emit_to_sid(self):
        sid = await self.bm.connect("123", "/foo")
        await self.bm.connect("456", "/foo")
        await self.bm.emit("my event", {"foo": "bar"}, namespace="/foo", to=sid)
        assert self.bm.server._send_eio_packet.await_count == 1
        assert self.bm.server._send_eio_packet.await_args_list[0][0][0] == "123"
    ...
    async def test_emit_to_room(self):
        ...
        await self.bm.connect("789", "/foo")
        await self.bm.emit("my event", {"foo": "bar"}, namespace="/foo", room="bar")
        assert self.bm.server._send_eio_packet.await_count == 2
        assert self.bm.server._send_eio_packet.await_args_list[0][0][0] == "123"
        assert self.bm.server._send_eio_packet.await_args_list[1][0][0] == "456"
        pkt = self.bm.server._send_eio_packet.await_args_list[0][0][1]
        assert self.bm.server._send_eio_packet.await_args_list[1][0][1] == pkt
        assert pkt.encode() == '42/foo,["my event",{"foo":"bar"}]'
    ...
    async def test_emit_to_rooms(self):
        ...
        assert self.bm.server._send_eio_packet.await_args_list[0][0][0] == "123"
        assert self.bm.server._send_eio_packet.await_args_list[1][0][0] == "456"
        assert self.bm.server._send_eio_packet.await_args_list[2][0][0] == "789"
        pkt = self.bm.server._send_eio_packet.await_args_list[0][0][1]
        assert self.bm.server._send_eio_packet.await_args_list[1][0][1] == pkt
        assert self.bm.server._send_eio_packet.await_args_list[2][0][1] == pkt
        assert pkt.encode() == '42/foo,["my event",{"foo":"bar"}]'
    ...
    async def test_emit_to_all_skip_one(self):
        ...
        await self.bm.connect("789", "/foo")
        await self.bm.connect("abc", "/bar")
        await self.bm.emit("my event", {"foo": "bar"}, namespace="/foo", skip_sid=sid2)
        assert self.bm.server._send_eio_packet.await_count == 2
        assert self.bm.server._send_eio_packet.await_args_list[0][0][0] == "123"
        assert self.bm.server._send_eio_packet.await_args_list[1][0][0] == "789"
        pkt = self.bm.server._send_eio_packet.await_args_list[0][0][1]
        assert self.bm.server._send_eio_packet.await_args_list[1][0][1] == pkt
        assert pkt.encode() == '42/foo,["my event",{"foo":"bar"}]'
    ...
    async def test_emit_to_all_skip_two(self):
        ...
        assert self.bm.server._send_eio_packet.await_count == 1
        assert self.bm.server._send_eio_packet.await_args_list[0][0][0] == "456"
        pkt = self.bm.server._send_eio_packet.await_args_list[0][0][1]
        assert pkt.encode() == '42/foo,["my event",{"foo":"bar"}]'
    ...
    async def test_emit_with_callback(self):
        ...
        await self.bm.emit("my event", {"foo": "bar"}, namespace="/foo", callback="cb")
        self.bm._generate_ack_id.assert_called_once_with(sid, "cb")
        assert self.bm.server._send_packet.await_count == 1
        assert self.bm.server._send_packet.await_args_list[0][0][0] == "123"
    ...
    async def test_emit_to_invalid_room(self):
        await self.bm.emit("my event", {"foo": "bar"}, namespace="/", room="123")
    ...
    async def test_emit_with_tuple(self):
        sid = await self.bm.connect("123", "/foo")
        await self.bm.emit("my event", ("foo", "bar"), namespace="/foo", room=sid)
        assert self.bm.server._send_eio_packet.await_count == 1
        assert self.bm.server._send_eio_packet.await_args_list[0][0][0] == "123"
        pkt = self.bm.server._send_eio_packet.await_args_list[0][0][1]
        assert pkt.encode() == '42/foo,["my event","foo","bar"]'
    ...
    async def test_emit_with_none(self):
        sid = await self.bm.connect("123", "/foo")
        await self.bm.emit("my event", None, namespace="/foo", room=sid)
        assert self.bm.server._send_eio_packet.await_count == 1
        assert self.bm.server._send_eio_packet.await_args_list[0][0][0] == "123"
        pkt = self.bm.server._send_eio_packet.await_args_list[0][0][1]
    ...
    async def test_emit_binary(self):
        sid = await self.bm.connect("123", "/")
        await self.bm.emit("my event", b"my binary data", namespace="/", room=sid)
        assert self.bm.server._send_eio_packet.await_count == 2
        assert self.bm.server._send_eio_packet.await_args_list[0][0][0] == "123"
        pkt = self.bm.server._send_eio_packet.await_args_list[0][0][1]
        assert pkt.encode() == '451-["my event",{"_placeholder":true,"num":0}]'
        assert self.bm.server._send_eio_packet.await_args_list[1][0][0] == "123"
