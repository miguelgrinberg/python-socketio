...
from socketio import async_namespace, async_server, exceptions, namespace, packet
...
class TestAsyncServer:
    ...
    async def test_create(self, eio):
        ...
        s = async_server.AsyncServer(client_manager=mgr, async_handlers=True, foo="bar")
    ...
    async def test_call_without_async_handlers(self, eio):
        mgr = self._get_mock_manager()
        s = async_server.AsyncServer(client_manager=mgr, async_handlers=False)
    ...
    async def test_send_packet(self, eio):
        ...
        s.eio.send.assert_awaited_once_with("123", '2/foo,["my event","my data"]')

    async def test_send_eio_packet(self, eio):
        eio.return_value.send = mock.AsyncMock()
        s = async_server.AsyncServer()
        await s._send_eio_packet("123", eio_packet.Packet(eio_packet.MESSAGE, "hello"))
    ...
    async def test_handle_connect_rejected_with_exception(self, eio):
        ...
        assert not s.manager.is_connected("1", "/")
        handler.assert_called_once_with("1", "environ")
        s.eio.send.assert_awaited_once_with("123", '4{"message":"fail_reason"}')
        assert s.environ == {"123": "environ"}
    ...
    async def test_handle_connect_namespace_rejected_with_exception(self, eio):
        ...
        s = async_server.AsyncServer()
        handler = mock.MagicMock(
            side_effect=exceptions.ConnectionRefusedError("fail_reason", 1, "2")
        )
        s.on("connect", handler, namespace="/foo")
        await s._handle_eio_connect("123", "environ")
        await s._handle_eio_message("123", "0/foo,")
    ...
    async def test_handle_connect_namespace_rejected_with_empty_exception(self, eio):
        eio.return_value.send = mock.AsyncMock()
        s = async_server.AsyncServer()
        handler = mock.MagicMock(side_effect=exceptions.ConnectionRefusedError())
        s.on("connect", handler, namespace="/foo")
    ...
    async def test_handle_disconnect(self, eio):
        ...
        s.manager.disconnect.assert_awaited_once_with("1", "/", ignore_queue=True)
        assert s.environ == {}
    ...
    async def test_handle_event(self, eio):
        ...
        catchall_handler.assert_awaited_once_with("my message", sid, "a", "b", "c")
    ...
    async def test_handle_event_with_namespace(self, eio):
        ...
        catchall_handler.assert_called_once_with("my message", sid, "a", "b", "c")
    ...
    async def test_handle_event_with_catchall_namespace(self, eio):
        ...
        await s._trigger_event(
            "disconnect", "/bar", sid_bar, s.reason.CLIENT_DISCONNECT
        )
    ...
    async def test_handle_event_with_ack(self, eio):
        ...
        s.eio.send.assert_awaited_once_with("123", '31000["foo"]')
    ...
    async def test_handle_event_with_ack_tuple(self, eio):
        ...
        s.eio.send.assert_awaited_once_with("123", '31000[1,"2",true]')
    ...
    async def test_send_with_ack(self, eio):
        ...
        await s._send_packet(
            "123", packet.Packet(packet.EVENT, ["my event", "foo"], id=id1)
        )
        await s._send_packet(
            "123", packet.Packet(packet.EVENT, ["my event", "bar"], id=id2)
        )
